...
---
### For contributors running inside the Jazz monorepo
If you’re running `npx jazz-run sync` from inside the Jazz repository itself (not a separate app), npm will try to use the local workspace version of jazz-run.

To fix:
1. Add jazz-run as a devDependency of the workspace:

<CodeGroup>
```sh
pnpm add -D jazz-run@workspace:*
```
</CodeGroup>
Then rerun:
