...
[Better Auth](https://better-auth.com/) is a framework-agnostic authentication and authorization framework for TypeScript. Jazz can be integrated with Better Auth for the authentication phase, to store the [Jazz's Account keys](./overview) on Better Auth's users.
Despite other cloud solutions, Better Auth is self-hosted.
...
<Alert variant="warning" className="mt-4">
  **Note**: the server-side plugin intercepts the custom header `x-jazz-auth` sent by client-side plugin. If server is behind a proxy, the header must be forwarded. If the server runs on a different origin than the client, the header must be allowed for cross-origin requests.
</Alert>
...
Here is how to signUp with email and password, and transform the Jazz's anonymous account into a logged in user:
...
And here is how to signIn with email and password and signOut:
...
Although Better Auth is not fully local-first, the Jazz's client plugin tries to keep the Jazz's authentication state in sync with Better Auth's. The best practice to check if the user is authenticated is using the Jazz's methods [described here](./authentication-states#detecting-authentication-state).

If you want to check the Better Auth's state, you can use its [native methods](https://www.better-auth.com/docs/basic-usage#session).


## Server-side hooks

Better Auth provides [database hooks](https://www.better-auth.com/docs/reference/options#databasehooks) to run code when things happen. When using Jazz's plugin, the Jazz's AccountID is always available in the `user` object. This means you can access it in any part of Better Auth's hooks.
