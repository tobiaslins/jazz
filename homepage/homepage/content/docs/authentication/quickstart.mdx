...
Jazz has a built-in passkey authentication component that you can use to add authentication to your app. This is the easiest way to get started with securely authenticating users into your application.
...
Run your app and try creating a passkey and logging in!
<RunDevServer />
...
<ContentByFramework framework="react">
  <FileName>app/invite/page.tsx</FileName>
</ContentByFramework>
<ContentByFramework framework="svelte">
  <FileName>src/routes/invite/+page.svelte</FileName>
</ContentByFramework>
...
<script lang="ts">
  ...
  // [!code ++:2]
  const festivalId = id ?? me?.root.myFestival.$jazz.id;
  const festival = new CoState(Festival, festivalId);
</script>
...
There's more to follow here on how to server-side render initially and then hydrate the client based on the user's authentication state.
</details>
</ContentByFramework>
