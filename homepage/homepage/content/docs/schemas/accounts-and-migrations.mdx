...
export class MyAppAccount extends Account {
  root = coField.ref(MyAppRoot);
}

export class MyAppRoot extends CoMap {
  myChats = coField.ref(ListOfChats);
  myContacts = coField.ref(ListOfAccounts);
}
...
export class MyAppAccount extends Account {
  profile = coField.ref(MyAppProfile); // [!code ++]
  root = coField.ref(MyAppRoot);
}
...
export class MyAppProfile extends Profile { // [!code ++:4]
  name = coField.string; // compatible with default Profile schema
  avatar = coField.optional.ref(ImageDefinition);
}
