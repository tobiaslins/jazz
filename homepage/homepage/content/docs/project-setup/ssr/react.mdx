...
<Alert variant="info" className="mt-4 flex gap-2 items-center">
  This guide builds on the [React Quickstart](/docs/getting-started/quickstart/react) to show you how to set up SSR to render public data on the server. Although it should be possible to follow this guide without having completed the quickstart, it's recommended to do so first.
</Alert>
