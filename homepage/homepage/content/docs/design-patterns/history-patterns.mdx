...
function findCompletionTime(task: Task): Date | null {
  ...
  // ⚠️ Note: .find() returns the FIRST completion time
...
// To get the LATEST completion time instead:
function findLatestCompletionTime(task: Task): Date | null {
  if (!task._edits.status) return null;
  
  // Find all completion edits and get the most recent one
  const completionEdits = task._edits.status.all.filter(
    edit => edit.value === "completed"
  );
  
  if (completionEdits.length === 0) return null;
  
  // Get the latest completion
  const latestCompletion = completionEdits[completionEdits.length - 1];
  return latestCompletion.madeAt;
}
