...
function getAuditLog(task: Task) {
  ...
  for (const field of fields) {
    const editField = field as keyof typeof task.$jazz.getEdits();
    if (!task.$jazz.getEdits()[editField]) continue;

    for (const edit of task.$jazz.getEdits()[editField].all) {
      changes.push({
        field,
        value: edit.value,
        by: edit.by,
...
function getRecentActivity(projects: Project[], since: Date) {
  ...
  for (const project of projects) {
    ...
    for (const field of fields) {
      const editField = field as keyof typeof project.$jazz.getEdits();
      // Skip if no edits exist for this field
      if (!project.$jazz.getEdits()[editField]) continue;

      for (const edit of project.$jazz.getEdits()[editField].all) {
...
function getLastUpdated(task: Task) {
  ...
  for (const field of Object.keys(task)) {
    ...
    const fieldEdit = task.$jazz.getEdits()[editField];
...
function findCompletionTime(task: Task): Date | null {
  if (!task.$jazz.getEdits().status) return null;
  ...
  const completionEdit = task.$jazz.getEdits().status.all.find(
...
function findLatestCompletionTime(task: Task): Date | null {
  ...
  const latestCompletionEdit = task.$jazz.getEdits().status.all
...
function findWhoChanged(task: Task, field: string, value: any) {
  const edits = task.$jazz.getEdits()[field as keyof typeof task.$jazz.getEdits()];
