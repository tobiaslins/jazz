...
In a `BubbleTeaOrder`, the `name` field is required, so it would be a good idea to validate this before turning the partial order into a real order.

Update the schema to include a `validatePartialBubbleTeaOrder` helper.
...
export function validatePartialBubbleTeaOrder(order: PartialBubbleTeaOrder) { // [!code ++:9]
  const errors: string[] = [];
...
export function CreateOrder(props: { id: string }) {
  ...
  if (!newOrder || !me) return;

  const handleSave = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();

    const validation = validatePartialBubbleTeaOrder(newOrder); // [!code ++:5]
