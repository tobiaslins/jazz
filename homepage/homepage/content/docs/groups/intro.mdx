...
Because `.$jazz.owner` can be an `co.account` or a `Group`, in cases where you specifically need to use `Group` methods (such as for adding members or getting your own role), you can cast it to assert it to be a Group:
...
const group = existingCoValue.$jazz.owner.$jazz.castAs(Group);
group.addMember(bob, "writer");

const role = group.getRoleOf(bob.$jazz.id);
```
</CodeGroup>
