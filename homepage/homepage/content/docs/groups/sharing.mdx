...
class RequestsList extends CoList.Of(Request) {}

class RequestsToJoin extends CoMap {
  writeOnlyInvite = co.string
  requests = co.ref(Project);
}
...
function createRequestsToJoin() {
  ...
  return RequestsToJoin.create({ writeOnlyInvite, requests: RequestsList.create([], requestsGroup)
  },  everyoneReaderGroup)
}
```
</CodeGroup>
...
async function requestToJoin(id) {
   const requestsToJoin = await RequestsToJoin.load(id)
