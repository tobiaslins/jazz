...
<CodeGroup>
```ts twoslash
import { co, CoList, CoMap, ID, Account } from "jazz-tools";
import { createInviteLink, consumeInviteLink } from jazz-react;
...
async function requestToJoin(projectId: ID<Project>) {
   ...
   requests.push(RequestsToJoin?.create({ account: Account.getMe() }, requests._owner));
}
```
</CodeGroup>
