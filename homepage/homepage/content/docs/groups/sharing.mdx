...
Using the write-only invitation link, users can submit requests and administrators can review and approve requests.

<CodeGroup>
```ts twoslash
import { co, CoMap, CoList, ID, Account, Group } from "jazz-tools";
...
async function approveJoinRequest(joinRequest: JoinRequest, targetGroup: Group) {
  const account = joinRequest.account && await Account.load(joinRequest.account.id);
