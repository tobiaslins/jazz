...
You can accept the defaults for all the questions, or customize the project as you like.
...
## Define a Schema
Jazz uses Zod for more simple data types (like strings, numbers, booleans), and composes these with CoSchemas to create collaborative data structures known as CoValues. These collaborative data structures are synced in real-time with end-to-end encryption.

Here we're defining a schema composed of both Zod types and CoValues. We're also adding a migration which runs when the user logs in to ensure that the root is always defined. Without this, we would get an error when trying to access `myFestival` later.
<CodeGroup>
```ts
// app/schema.ts
...
Wrap your app with `JazzReactProvider` so components can use Jazz. Jazz also provides a basic UI for passkey-based authentication, which we'll use here.
<CodeGroup>
```tsx
// app/layout.tsx
"use client";
...
## Start your app
Moment of truth&thinsp;—&thinsp;time to start your app and see if it works.
