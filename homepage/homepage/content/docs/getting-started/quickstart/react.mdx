...
## Define a Schema
Jazz uses Zod to define primitive data types, and CoSchemas to define collaborative data type. We're adding a migration which runs when the user logs in to ensure that the root is always defined.
<CodeGroup>
```ts
// app/schema.ts
...
export const JazzFestAccount = co
  ...
  .withMigration((account) => {
    ...
    // because the root may be null (still loading)
...
Wrap your app with `JazzReactProvider` so components can use Jazz. Jazz also provides a basic UI for authentication with passkeys, which we'll use here.
<CodeGroup>
```tsx
// app/layout.tsx
"use client";
...
If everything's going according to plan, you should see a login prompt! No log in prompt? Ask for help on [Discord](https://discord.gg/utDMjHYg42).
...
export function MyFestival() {
  ...
  return (
    <ul>
      {me?.root.myFestival.map((band) => band && <li key={band.id}>{band.name}</li>)}
    </ul>
  );
}
```
...
- Learn how to share and collaborate on data