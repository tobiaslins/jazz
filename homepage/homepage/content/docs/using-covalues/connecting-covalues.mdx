...
export const ReferenceItem = co.map({
  ...
  // Workaround: declare the field type using co.List and co.DiscriminatedUnion so TS can safely recurse
  get children(): co.List<co.DiscriminatedUnion<[typeof NoteItem, typeof AttachmentItem, typeof ReferenceItem]>> {
