...
Consider a scenario where one needs to update a CoMap if it exists, or create it if it doesn't:
...
Note that `CoMap.upsertUnique` will [skip retrying unavailable CoValues](/docs/using-covalues/subscription-and-loading#retrying-unavailable-items) by default. If you expect the CoMap to eventually become available if it's not already, you can specify `skipRetry: false` as a load option:
