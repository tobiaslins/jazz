...
Identifying CoMaps by their CoValue IDs can be inconvenient if using an externally-generated identifier is preferrable, such as when:
...
- The CoMaps have human-readable & application-specific identifiers
  - If an application has CoValues used by every user, referring to it by a unique name (eg, `"my-global-comap"`) can be more convenient than using a CoValue ID
...
// ---cut---
// This will not work, as `sourceData.identifier` is some externally-generated string, not a CoValue ID
let activeEvent = await Event.load(sourceData.identifier);
```
</CodeGroup>

To achieve this, one could use `CoMap.loadUnique`:
...
Now suppose you wished to create or modify a CoMap using its unique identifier:
...
To simplify this, one could use `CoMap.upsertUnique`:
