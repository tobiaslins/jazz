...
While `co.string` is perfect for simple text fields that don't need collaboration, `CoPlainText` is the right choice when you need:

- Multiple users editing the same text simultaneously
- History tracking of text changes
- Fine-grained control over text edits (inserting, deleting at specific positions)
- Real-time updates across clients

Think of `CoPlainText` as a collaborative upgrade to `co.string` fields.
