...
## CoPlainText vs coField.string

While `coField.string` is perfect for simple text fields, `CoPlainText` is the right choice when you need:
...
<CodeGroup>
```ts twoslash
import { CoMap, CoPlainText, CoRichText, coField } from "jazz-tools";

// ---cut---
class Profile extends CoMap {
  name = coField.string;
  bio = coField.ref(CoPlainText);         // Plain text field
  description = coField.ref(CoRichText);  // Rich text with formatting
}
```
</CodeGroup>
...
import { CoRichText, Account, Profile, coField } from "jazz-tools";
