...
You can also include nested CoValues in your branch by using a [`resolve` query](/docs/using-covalues/subscription-and-loading#resolve-queries). When you create a branch, the branch selection automatically propagates to all sub-properties and nested CoValues:

- **Every sub-property** that comes from the branched CoValue will also be branched
- **All subscriptions** using `branch.$jazz.ensureLoaded()` and `branch.$jazz.subscribe()` will have the branch checkout applied to their results
- **Nested CoValues** will have their own branched versions created automatically
