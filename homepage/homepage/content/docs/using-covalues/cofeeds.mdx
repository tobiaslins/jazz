...
class Activity extends CoMap {
  ...
  action = coField.literal("watering", "planting", "harvesting", "maintenance");
  notes = coField.optional.string;
}

// Define a feed of garden activities
class ActivityFeed extends CoFeed.Of(coField.ref(Activity)) {}
...
<CodeGroup>
```ts twoslash
import { Group, coField, CoMap, CoFeed } from "jazz-tools";
