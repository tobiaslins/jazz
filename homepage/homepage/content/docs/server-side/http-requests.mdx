...
If you only need to authenticate requests and don't need the advanced features of [Jazz-managed requests](#jazz-managed-requests), generate a short-lived token with `generateAuthToken` and verify it on the server with `authenticateRequest`.
...
Authenticating a request without a token doesn't throw an error; it returns `account` as `undefined`. For endpoints that require authentication, ensure `account` is defined in addition to any permission checks you perform.
