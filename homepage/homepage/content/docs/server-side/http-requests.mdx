...
<Alert variant="info" className="mt-4">
  **Note**: To ensure that the rule is properly followed, the handler should be deployed in a single worker instance (e.g. a single Cloudflare DurableObject).
  
  Transactional updates are part of the roadmap, but not yet supported.
</Alert>
