...
3. Rate limiting or quota tracking
   * Enforcing a user can only perform 10 actions per day.
   * Needs atomic updates to the counter to avoid bypassing the limit.
4. Sequential ID or token generation
...
As a simple rule to follow, when the output of the request depends on the state of the database, you should probably deploy on a single instance.
