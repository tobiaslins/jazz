...
If you only need to authenticate requests, and don't need any of the advanced features of [Jazz-managed requests](#jazz-managed-requests), you can use the `generateAuthToken` to generate an authentication token that can be used to authenticate the request using the `authenticateRequest` function.

### Creating a Request

You can use any method to create a request, but the most common way is to use the `fetch` function.
...
Note that attempting to authenticate a request without a token will not return an error, but an `undefined` account. If you need to restrict access to reseources that should be accessible only to authenticated users, you should check for the `account` being `undefined`, on top of any additional permission checks you might need.
...
export async function GET(request: Request) {
  ...
  // There was an error validating the token. I.e. the token is invalid or expired
  if (error) {
...
### Multi-accounts environments

If you are using multiple accounts in your environment, or in general if you need to send and  authenticate requests as a specific accounts you can specify the account to use when generating the token or when authenticating the request.
...
Similarly, you can specify the account to verify the token with by using the `loadAs` option:
...
While using the `Authorization` header is the most common way to send the token, you can provide the token in any other way you want.
...
## Jazz-managed requests

Jazz-managed requests are a more advanced way to make requests. See the [Security](#security-safeguards-provided-by-jazz) section for more details on the benefits and guarantees they provide compared to regular HTTP requests.
