...
It works on top of the Jazz APIs and uses the sync to transfer the messages between the client and the server.
...
Run a server worker with and subscribe to the `inbox`:
