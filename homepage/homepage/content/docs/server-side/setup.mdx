...
To use Node-API (native crypto) with Next.js on the server (for example, in a Vercel Function), you need to tell Next.js to 
include the required native modules. Add them to the [`serverExternalPackages`](https://nextjs.org/docs/app/api-reference/config/next-config-js/serverExternalPackages) array in your `next.config.js`. 

This step is necessary for Next.js to bundle and deploy the Node-API implementation correctly. As example:
