...
The main difference to keep in mind when working with Jazz compared to traditional systems, is that servers code doesn't have any
special or privileged access to the user data, and that you need to be explicit about what you want to share with the server.
...
The migration definied in the `AccountSchema` will be executed every time the worker starts, the same way as it would be for a client-side Jazz context.
...
It will be implicitily become the current account, and you can avoid mentioning it in most cases.

For this reason we also recommend to run a single worker instance per server, because it makes your code way more predictable.

## Storing & providing credentials

Server Worker credentials are typically stored and provided as env variables.
