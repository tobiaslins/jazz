...
export class MyAppAccount extends Account {
  static async migrate(
    me: MyAppAccount,
    creationProps: { name: string } // from AuthMethod
  ) {
    // we specifically need to check for undefined, because the profile might simply be not loaded (`null`) yet
    if (me.profile === undefined) {
      // this is the default profile setup migration and if you don't need anything else, you can leave out this whole block
      const publiclyReadableGroup = Group.create({ everyone: "reader" });
      me.profile = Profile.create({ name: creationProps.name }, { owner: publiclyReadableGroup });
    }
    // we specifically need to check for undefined, because the profile might simply be not loaded (`null`) yet
    if (me.root === undefined) {
      me.root = MyAppRoot.create({
        myChats: ListOfChats.create([], { owner: me }),
        myContacts: ListOfAccounts.create([], { owner: me })
      });
    }
  }
}
...
TODO: code example of complex migration

### Best practice: only ever add fields

TODO: explain and reassure that there will be more guardrails in the future