...
First you define a context providing component (typically called `JazzAndAuth`) that uses:
    ...
    // Use this snippet to register your app's Account schema so the Jazz hooks are typed with it
...
<CodeGroup>
  ...
  ```tsx
  "use client"
  import { JazzProvider, PasskeyAuthBasicUI, usePasskeyAuth } from "jazz-react";// old
