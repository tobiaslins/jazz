...
You may want allow your users to use your app without authenticating (a poll response for example). But they might make changes that they'd like to keep when they do authenticate.

When they log in, the anonymous account will be deleted and this could lead to unexpected data loss.
...
You can disable network sync so you can decide when it makes sense for your app to work completely locally.

For example, you may want to give the opportunity to non-authenticated users to try your app locally first, then enable the network sync only when the user is authenticated:
<CodeGroup>
```tsx
<JazzProvider
  sync={{
...
You can manually control the sync by switching when between `"always"` and `"never"`.
