...
When a user loads for the first time a Jazz application, we generate those keys and store them locally.

Without any additional steps the user can use Jazz normally, but it would be limited to use only one device.

To expand the usage of the account to multiple devices and provide the credentials backup you can setup a signUp method using one of the Jazz's authentication providers.
...
The passphrase authentication is based on giving to the user a passphrase to use to log in with another device.
...
The clerk provider is not built-in into `jazz-react` and needs the `jazz-react-auth-clerk` package to be installed.
