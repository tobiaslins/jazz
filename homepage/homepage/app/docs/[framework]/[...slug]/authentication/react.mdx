...
- The user loads the app:
    - If it's the first time a new anonymous account is created
    - Otherwise the last used account is loaded from the local storage
- The user tries to authenticate:
    - When the user performs a signIn, the current account credentials are registered in the selected provider
    - If instead logs-in the local credentials are deleted and the authenticated account is loaded
...
const { state, logIn, signUp } = usePasskeyAuth({ appName });
```
</CodeGroup>

Take a look at our [example](https://github.com/garden-co/jazz/tree/main/examples/passkey) to see how it works.
...
const { state, logIn, signUp, getCurrentAccountPassphrase } = usePassphraseAuth({ wordlist });
```
</CodeGroup>
...
Take a look at our [example](https://github.com/garden-co/jazz/tree/main/examples/passphrase) to see how it works.
...
3. Add `jazz-react-auth-clerk` to your project.
...
5. Use the [Clerk auth methods](https://clerk.com/docs/references/react/overview) to log in and sign up.
