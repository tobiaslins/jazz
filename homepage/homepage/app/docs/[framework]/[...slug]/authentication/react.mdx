...
For example, you may want to give the opportunity to non-authenticated users to try your pp locally first, then enable the network sync only when the user is authenticated:
<CodeGroup>
```tsx
<JazzProvider
  sync={{
