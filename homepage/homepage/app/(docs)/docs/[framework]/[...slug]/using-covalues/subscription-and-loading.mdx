...
The load operation will fail and return `undefined` if any requested reference is inaccessible. This maintains data consistency by ensuring all requested references are available before returning the object.
...
If any item in a list is inaccessible to the user, the entire load operation will fail and return `undefined`. This is because lists expect all their items to be accessible - a partially loaded list could lead to data inconsistencies.

#### Accessing an inaccessible reference via autoload
