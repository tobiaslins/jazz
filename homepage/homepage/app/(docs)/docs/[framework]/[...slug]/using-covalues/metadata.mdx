...
`CoValue._raw` gives you access to the internal state of a CoValue, from which a lot of useful information can be derived. As this is an internal API, it should be used with caution. If you find yourself using `_raw`, consider letting us know so we can consider adding a public API for your use case.
...
function getRecentActivity(project: Project) {
  ...
  for (const field of Object.keys(project)) {
    for (const edit of project._raw.editsAt(field)) {
      if (edit.at > hourAgo) {
        activity.push({
          field,
          value: edit.value,
...
recentActivity.forEach(activity => {
  console.log(`${activity.at.toLocaleString()} - ${activity.field} updated by ${activity.byID}`);
});
```
</CodeGroup>
