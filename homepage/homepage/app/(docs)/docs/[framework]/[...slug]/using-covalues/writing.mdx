...
Write operations can fail for several reasons, such as permission issues or network problems. It's important to handle these errors gracefully to provide good user experience. Here's how you can catch and process write errors:

<CodeGroup>
```ts
// TODO: Test this or verify it's correct
try {
  task.status = "completed";
} catch (error) {
  if (error.message.includes("permission")) {
    console.error("Not authorized to update task status");
  } else {
