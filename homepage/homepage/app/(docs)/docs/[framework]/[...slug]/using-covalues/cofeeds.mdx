...
CoFeeds are collaborative, append-only data structures that allow multiple accounts and sessions to contribute entries. Unlike CoMaps and CoLists which provide shared collaborative state, CoFeeds organize data by user and session, making them ideal for tracking user presence, activity logs,  notifications, and other session-based information.

Each account can have multiple sessions (different browser tabs, devices, or app instances), and each session maintains its view of the feed. This design allows you to track what different users are doing across their various devices and sessions.
