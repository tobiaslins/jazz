...
Anonymous Authentication can be [upgraded to an Authenticated Account](./overview#migrating-data-from-anonymous-to-authenticated-account), preserving user data. Once authenticated, users can access their account across multiple devices.

Without additional authentication steps beyond Anonymous Authentication, users can use Jazz normally but are limited to using the application on only one device.
...
- **Login**: When a user logs in with an authentication provider, their Anonymous account is discarded and the credentials are loaded from the auth provider. Data from the Anonymous account [can be transferred to the authenticated account](./overview#migrating-data-from-anonymous-to-authenticated-account).
