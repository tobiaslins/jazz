...
```tsx twoslash
// @noErrors: 2686
import { SignInButton, useSignIn } from "@clerk/clerk-expo";
...
<JazzProvider
  ...
  guestMode={true}
  // Optional: Configure collections accessible in guest mode
  publicCollections={['publicContent', 'readOnlyResources']}
  sync={{
    peer: `wss://cloud.jazz.tools/?key=${apiKey}`,
